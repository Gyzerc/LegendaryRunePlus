...
public class UnlockCommand extends com.gyzer.Commands.Builder.CommandProvider {
        ...
        if (target != null) {
            ...
            if (page != null) {
                if (page.getRuneSlotIds().contains(slotId)) {
                    sender.sendMessage(configManager.lang_plugin + configManager.lang_unlock_slot_admin.replace("%player%",args[1]).replace("%page%",pageId).replace("%slot%",slotId));
                    UserData data = legendaryRunePlus.getUserDataManager().getUserData(target.getUniqueId());
                    if (!data.hasUnlockSlot(pageId,slotId) ) {
                        data.unlockSlot(target,pageId,slotId);
                        target.sendMessage(configManager.lang_plugin + configManager.lang_unlock_slot.replace("%slot%",page.getRuneSlotDisplayById(slotId)));
                    }
                    return;
                }
